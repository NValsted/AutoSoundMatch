...
class PolyDataset(Dataset):
    ...
    def __init__(self, db: Database, test_flag: bool = False):
        ...
        with self.db.session() as session:
            query = select(AudioBridgeTable)\
                .where(AudioBridgeTable.test_flag == test_flag)
    ...
    def _process_signal(self, signal: np.ndarray, sample_rate: int):
        mel_spectrogram = librosa.feature.melspectrogram(
            y=signal, sr=sample_rate, n_fft=2048, n_mels=128, hop_length=1024, fmin=30, fmax=11000
        )
        return librosa.power_to_db(mel_spectrogram)

    def __getitem__(self, index) -> Tuple[torch.Tensor, np.ndarray, np.ndarray, np.ndarray]:
        with self.db.session() as session:
            audio_bridge = self.audio_bridges[index]

            query = select(SynthParamsTable.__table__)\
                .where(SynthParamsTable.id == audio_bridge.synth_params)
