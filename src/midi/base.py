...
from scipy.stats import _continuous_distns as continuous_distributions
from scipy.stats import _discrete_distns as discrete_distributions
...
class ASMMidiNote(BaseModel):
    ...
    def valid_midi_value(cls, value):
        if value < 0 or value > 127:
            raise ValueError(
                f"MIDI value must be between 0 and 127 (inclusive). Got {value=}"
            )
        return value
    ...
    def random_from_scale(
        ...
        if isinstance(scale, str):
            if scale not in SCALES:
                raise ValueError(
                    f"{scale} is not a valid scale. Valid scales are"
                    f" {', '.join(SCALES.keys())}"
                )
            scale = SCALES[scale]

        note = (
            base_note + (random.choice(scale) - 1) + int(octave_distribution.rvs()) * 12
        )
    ...
    def on_message(self):
        ...
        """
        return Message(
            "note_on", note=self.note, velocity=self.velocity, time=self.offset
        )
...
class ASMMidiTrack(MidiTrack):
    ...
    def from_notes(cls, notes: list[ASMMidiNote]) -> "ASMMidiTrack":
        ...
        messages = [
            message
            for note in notes
            for message in (note.on_message(), note.off_message())
        ]
